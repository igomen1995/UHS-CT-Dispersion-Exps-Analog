...
%             ylim(ax5,[-0.02 1])
%             xlim(ax5,[tmin,tmax])
%             ticks = get(ax5,'XTick'); % seconds
%             L = x2(end); % cm
%             vel = expCTData.(filedataExp.Key(i)).CFparams.velFront_cmmin; % cm/min
%             tD_ticks = vel * (ticks/60) / L; 
%             set(ax5b,'XTick', ticks, 'XTickLabel', compose('%.2f', tD_ticks)); % update top axis
...
for i = 1:length(filedataExp.Key)
    ...
    L = expCTData.(filedataExp.Key(i)).concVarsAll.C1Profile{1,1}(end,1);
    vel = expCTData.(filedataExp.Key(i)).CFparams.velFront_cmmin;
